...
| pending current-thought purpose | Agent는 `check_current_thought({})`로 시점만 선택하고 서버가 고정 질문을 표시한다. 목적은 라이브 runtime에만 기록하며, help 교환만 있는 후속 TURN의 Agent가 Assessor routing에 사용하고 RESTORE에서는 전체 시간순 대화로 복구한다. |
...
현재 생각 확인 목적은 외부 route·DTO·snapshot·DB 상태가 아니라 runtime 내부 routing 메타데이터다. Agent는 확인 시점만 선택하고 표시 문구는 서버 상수 `그렇다면, 처음에 떠올랐던 판단을 지금은 어떻게 보고 있나요?`를 사용한다. 이 메타데이터는 AFTER 성립 여부를 판정하지 않으며, 실제 USER 답변을 Assessor에게 넘길 시점만 알려 준다. 프로세스 복원 뒤에는 Agent가 저장된 전체 ASSISTANT/USER 이력을 역추적한다. 확인 질문 뒤에 도움·설명·예시 교환만 있으면 결속을 회복하고, 안전·명시적 제어·주제 포기/전환·일반 질문·proposal 전이가 있으면 회복하지 않는다. 새 영속 상태, 추가 LLM·Writer·verifier, 의미 regex/keyword gate와 retry는 추가하지 않는다.
