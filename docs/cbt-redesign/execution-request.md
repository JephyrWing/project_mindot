...
- 도구는 `ask_question(text)`, `offer_help(text)`, `assess_completion()`, `respond_control()`, `respond_safety(action, reason)`다.
...
- 활성 `cbt_simple`은 `cbt_q11`·`cbt_agent.py`를 import하지 않는다.

## 리뷰 범위

1. `app.py → cbt_session_agent.py → cbt_simple`의 실제 import와 호출 closure.
...
8. runtime model-contract/prompt와 `docs/cbt-redesign` 사본의 동일성.
