...
public class EmotionRecordsService {
    ...
    private final ReflectionSessionEmbeddingTransactionService embeddingTransactions;
    ...
    public EmotionRecordsPageResponseDto getEmotionRecords(
        ...
        // Explicit CASE keeps missing intensity last with Criteria pagination.
        boolean intensitySort = sort == EmotionRecordsListSort.INTENSITY_HIGH || sort == EmotionRecordsListSort.INTENSITY_LOW;
        if (intensitySort) specification = specification.and((root, query, builder) -> {
            if (query != null && query.getResultType() != Long.class) query.orderBy(
                    builder.asc(builder.<Integer>selectCase().when(builder.isNull(root.get("primaryIntensity")), 1).otherwise(0)),
                    sort == EmotionRecordsListSort.INTENSITY_HIGH ? builder.desc(root.get("primaryIntensity")) : builder.asc(root.get("primaryIntensity")),
                    builder.desc(root.get("occurredAt")), builder.desc(root.get("id")));
        ...
        Page<EmotionRecords> emotionRecordsPage =
                emotionRecordsRepository.findAll(
                        ...
                        PageRequest.of(
                                page,
                                size,
                                intensitySort ? org.springframework.data.domain.Sort.unsorted() : sort.toSort()
                        )
                );
    ...
    private PeriodRange calculatePeriodRange(
        ...
        return switch (period) {
            ...
            case RECENT_7_DAYS -> new PeriodRange(today.minusDays(6).atStartOfDay(userTimezone).toInstant(),
                    today.plusDays(1).atStartOfDay(userTimezone).toInstant());
