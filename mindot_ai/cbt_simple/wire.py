...
def messages(phase,payload,pair=()):
    ...
    context=dict(record=snapshot['record'],phase=snapshot['phase'],currentProposal=snapshot.get('currentProposal'),
        historicalTypeReviews=snapshot.get('historicalTypeReviews',[]),moderation=snapshot.get('moderation'),distortionDefinitions=schema.DEFINITIONS)
    ...
    for row in snapshot['messages']:
        ...
        result.append(cls(content=canonical(dict(messageNumber=row['messageNumber'],content=row['content']))))
    return [*result,*pair]
