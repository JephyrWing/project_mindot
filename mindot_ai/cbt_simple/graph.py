...
def base(request,question=None,*,risk=None,result=None,ack=None,evidence_for=None,evidence_against=None):
    ...
    return CbtTurnResponse(request_id=request.request_id,
        ...
        meta=AnalysisMeta(model='gpt-4o-mini',prompt_version='simple-dialogue-2'))
...
def question_response(request,state,message,plan,*,control=None,risk=None):
    ...
    target=state['questions'].get(plan.get('targetQuestionCode')) if plan.get('mode') in ('EXAMPLE','EXPLANATION') or control=='WAIT' else None
...
def compile_graph(saver,request,provider,diagnostics,moderation):
    async def select(g):
        msg,selection=await provider.choose(provider.messages('SELECT',dict(view=st.view(g['accepted']),moderation=moderation)))
    ...
    async def write_turn(state,args):
        plan=dict(move=args['move'],focus=args['focus'],sourceIds=args['sourceIds'],mode='NORMAL')
    ...
    async def review(g):
        ...
        pair=g['messages'][-2:]
        msg,decision=await provider.review(provider.messages('ASSESSMENT_REVIEW',dict(view=st.view(state,1)),pair))
