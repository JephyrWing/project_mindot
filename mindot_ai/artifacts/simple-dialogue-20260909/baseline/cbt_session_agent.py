"""Coverage Agent와 Completion Assessor를 분리한 Q10 CBT runtime입니다.
...
CBT_AGENT_PROMPT_VERSION = "cbt-session-agent-quality-q10-coverage"
CBT_ASSESSOR_PROMPT_VERSION = "cbt-session-agent-quality-q10-assessor"
CBT_WRITER_PROMPT_VERSION = "cbt-session-agent-quality-q9-writer-preserved"
...
CBT_ASSESSOR_MODEL_OUTPUT_ATTEMPTS = int(
    os.getenv("CBT_ASSESSOR_MODEL_OUTPUT_ATTEMPTS", "2")
)
...
class AskQuestionAction(ApiModel):
    """Coverage가 미완료일 때 다음 질문 하나를 계획합니다."""

    coverage_review: CoverageReview = Field(alias="coverageReview")
    question_plan: AgentQuestionPlan = Field(alias="questionPlan")


class RequestExampleAction(ApiModel):
    """예시 요청에 중립 선택지 두 개만 계획합니다."""

    coverage_review: CoverageReview = Field(alias="coverageReview")
    question_plan: AgentQuestionPlan = Field(alias="questionPlan")


class ReadyForAssessmentAction(ApiModel):
    """서버 병합 후 네 coverage domain이 complete라고 제안합니다."""

    coverage_review: CoverageReview = Field(alias="coverageReview")


class UserSelectedDirectionAction(ApiModel):
    """반복·거부 누적 뒤 사용자가 방향을 고를 질문을 계획합니다."""
...
CoverageAgentAction = (
    AskQuestionAction
    | RequestExampleAction
    | ReadyForAssessmentAction
    | UserSelectedDirectionAction
)


class DistortionPresentAssessment(ApiModel):
    assessment_type: Literal["DISTORTION_PRESENT"] = Field(alias="assessmentType")
    observed_fact: str = Field(alias="observedFact", min_length=1, max_length=1_000)
    automatic_thought_excerpt: str = Field(alias="automaticThoughtExcerpt", min_length=1, max_length=1_000)
    unsupported_extension: str = Field(alias="unsupportedExtension", min_length=1, max_length=1_000)
    matched_distortion_code: DistortionCode = Field(alias="matchedDistortionCode")
    matched_definition_element: str = Field(alias="matchedDefinitionElement", min_length=1, max_length=1_000)
    definition_match_reason: str = Field(alias="definitionMatchReason", min_length=1, max_length=1_000)
    grounding_question_codes: list[str] = Field(alias="groundingQuestionCodes", min_length=1, max_length=8)
    preserved_fact: str = Field(alias="preservedFact", min_length=1, max_length=1_000)
    calibrated_thought: str = Field(alias="calibratedThought", min_length=1, max_length=1_000)


class NoClearConsideredCandidate(ApiModel):
    code: DistortionCode
    definition_element_checked: str = Field(alias="definitionElementChecked", min_length=1, max_length=1_000)
    missing_definition_element: str = Field(alias="missingDefinitionElement", min_length=1, max_length=1_000)
    why_automatic_thought_does_not_match: str = Field(alias="whyAutomaticThoughtDoesNotMatch", min_length=1, max_length=1_000)


class NoClearDistortionAssessment(ApiModel):
    assessment_type: Literal["NO_CLEAR_DISTORTION"] = Field(alias="assessmentType")
    observed_fact: str = Field(alias="observedFact", min_length=1, max_length=1_000)
    automatic_thought_excerpt: str = Field(alias="automaticThoughtExcerpt", min_length=1, max_length=1_000)
    within_fact_boundary_reason: str = Field(alias="withinFactBoundaryReason", min_length=1, max_length=1_000)
    considered_candidates: list[NoClearConsideredCandidate] = Field(alias="consideredCandidates", max_length=4)
    grounding_question_codes: list[str] = Field(alias="groundingQuestionCodes", min_length=1, max_length=8)
    preserved_fact: str = Field(alias="preservedFact", min_length=1, max_length=1_000)
    calibrated_thought: str = Field(alias="calibratedThought", min_length=1, max_length=1_000)


class AssessmentGap(ApiModel):
    missing_fact_boundary_information: str = Field(alias="missingFactBoundaryInformation", min_length=1, max_length=1_000)
    grounding_question_codes: list[str] = Field(alias="groundingQuestionCodes", min_length=1, max_length=8)
    why_existing_answers_are_insufficient: str = Field(alias="whyExistingAnswersAreInsufficient", min_length=1, max_length=1_000)
    distinction_needed: str = Field(alias="distinctionNeeded", min_length=1, max_length=1_000)


class NeedsMoreExplorationAssessment(ApiModel):
    assessment_type: Literal["NEEDS_MORE_EXPLORATION"] = Field(alias="assessmentType")
    assessment_gap: AssessmentGap = Field(alias="assessmentGap")
    recommended_route: SemanticRouteType = Field(alias="recommendedRoute")
    question_goal: str = Field(alias="questionGoal", min_length=1, max_length=300)
    answer_target: str = Field(alias="answerTarget", min_length=1, max_length=300)
    answer_source: Q5AnswerSource = Field(alias="answerSource")


CompletionAssessment = Annotated[
    DistortionPresentAssessment
    | NoClearDistortionAssessment
    | NeedsMoreExplorationAssessment,
...
class CompletionAssessmentEnvelope(ApiModel):
    """OpenAI 호환 top-level object이며 판정 branch는 assessment 아래에 둡니다."""

    assessment: ProviderCompletionAssessment


_COMPLETION_ASSESSMENT_ADAPTER = TypeAdapter(CompletionAssessment)
...
REQUEST_EXAMPLE_TOOL = StructuredTool.from_function(
    func=_tool_schema_only,
    name="request_example",
    description="Return coverageReview and exactly two neutral example options in one plan.",
    args_schema=RequestExampleAction,
)
READY_FOR_ASSESSMENT_TOOL = StructuredTool.from_function(
    func=_tool_schema_only,
    name="ready_for_assessment",
    description="Return only coverageReview when the expected server merge is complete.",
    args_schema=ReadyForAssessmentAction,
)
USER_SELECTED_DIRECTION_TOOL = StructuredTool.from_function(
    func=_tool_schema_only,
    name="user_selected_direction",
    description="Return coverageReview and one USER_SELECTED_DIRECTION plan.",
    args_schema=UserSelectedDirectionAction,
)
ACTION_SCHEMAS: dict[str, type[ApiModel]] = {
    ASK_QUESTION_TOOL.name: AskQuestionAction,
    REQUEST_EXAMPLE_TOOL.name: RequestExampleAction,
    READY_FOR_ASSESSMENT_TOOL.name: ReadyForAssessmentAction,
    USER_SELECTED_DIRECTION_TOOL.name: UserSelectedDirectionAction,
}


AGENT_SYSTEM_PROMPT = """
You plan coverage and the next dialogue action for one Korean CBT reflection turn. Call exactly one available tool. Do not diagnose, classify distortion, decide no-clear, write a calibrated thought, or produce final Korean wording.

AUTHORITY AND ORDER
Saved USER answers, sourceQuestionCodes, priorVerifiedCoverage, domainCandidates, blocked semantic signatures, rejected premises and the latest interaction are authoritative. Safety has already been handled by the server.
1. Review all four domains against saved answers.
2. Predict the server-valid merged coverage without downgrading prior verified state.
3. If any domain remains unresolved, choose one new answerable direction; if all are complete, submit readiness for assessment.
Tool availability and candidates are permission or review hints, never evidence.

COVERAGE
For evidenceFor, evidenceAgainst, alternativeViews and acknowledgement return only:
- EVIDENCE_FOUND with a sourceQuestionCode whose saved USER answer directly fits that domain;
- EXPLICITLY_NONE with an allowed explicit-none saved answer and sourceQuestionCode;
- NOT_EXPLORED when unresolved. If rejecting a supplied candidate, give a concrete rejectionReason.
Never invent excerpts, sources or state. NO_DIRECT_EVIDENCE closes only evidenceFor as EXPLICITLY_NONE. UNCLEAR, SKIPPED, silence, feedback and example requests fill no domain. Your review is a proposed delta; the server owns validation and monotonic merge.

ACTION
If the expected merged coverage is complete, choose READY_FOR_ASSESSMENT and return no question or assessment fields. You do not decide fact boundary or distortion.
If incomplete, ask for exactly one unresolved piece of information that can change readiness or later assessment. Ask what the user observed, experienced, judged, inferred or can consider; never require a third party's hidden state as fact. Purpose, route, goal, target/source and polarity must seek the same information.
Do not paraphrase a blocked route, target/source pair, semantic signature, resolved topic, skipped request, rejected premise or unanswered request. After one explicit SKIPPED, block that meaning. After two UNCLEAR attempts on the same meaning, choose USER_SELECTED_DIRECTION.
For REQUEST_EXAMPLE return exactly two short, neutral, situation-specific options and no final wording. Options are possibilities, not claims about a third party.

OUTPUT
Call exactly one available tool. Do not echo session state, downgrade prior coverage, return COMPLETE text, write final Korean wording, or include terminal assessment fields.
""".strip()


COMPLETION_ASSESSOR_SYSTEM_PROMPT = """
Assess the original automaticThought for one Korean CBT reflection after the server has verified all four coverage domains. Return exactly one structured assessment. Do not plan coverage, alter saved state, diagnose the user, or write a dialogue question.

DECISION ORDER
1. Identify observed facts supported by exact saved USER answers.
2. Compare the original automaticThought with those facts. Identify any added scope, certainty, prediction, causation, hidden-state inference, global label, emotional proof, discounting, magnification/minimization or rigid rule.
3. If existing answers cannot distinguish a specific close fact-boundary question, return NEEDS_MORE_EXPLORATION with one assessmentGap. Otherwise compare the added meaning with supplied distortion definitions.
4. If one definition is concretely present, return DISTORTION_PRESENT. Otherwise return NO_CLEAR_DISTORTION.

BOUNDARY RULES
Evaluate the original automaticThought, not only a later balanced thought. Preserve true negative facts and proportionate emotion. A thought may contain both a true fact and a distorted extension.
Lack of external evidence for the thought is not a reason to exclude a distortion. To exclude a candidate, name the definition element absent from the automaticThought. Uncertainty can itself support mind reading, fortune telling or another certainty-based extension.
Use only allowed, non-rejected codes and supplied definitions. Never re-propose a rejected code. Do not treat EXPLICITLY_NONE, UNCLEAR, SKIPPED, feedback or example requests as positive evidence.

OUTPUT TYPES
DISTORTION_PRESENT requires exact observedFact grounding, an exact non-empty automaticThoughtExcerpt, a non-empty unsupportedExtension, one allowed matchedDistortionCode, the matched definition element and reason, groundingQuestionCodes, preservedFact and a concrete believable calibratedThought.
NO_CLEAR_DISTORTION requires observedFact, exact automaticThoughtExcerpt, withinFactBoundaryReason, groundingQuestionCodes, preservedFact, calibratedThought and only close consideredCandidates. For each candidate state definitionElementChecked, missingDefinitionElement and whyAutomaticThoughtDoesNotMatch. No-clear is tentative, not proof the thought is true.
NEEDS_MORE_EXPLORATION is exceptional. Use it only when existing complete coverage still cannot distinguish two close boundaries. State the missing fact-boundary information, groundingQuestionCodes, why existing answers are insufficient, distinctionNeeded, recommendedRoute, questionGoal, answerTarget and answerSource. Do not use it for low confidence, repeated questioning or proving every distortion absent.

Return exactly one structured assessment and no user-facing prose.
""".strip()
...
class AgentSessionRuntime:
    ...
    pending_assessment_gap: AssessmentGap | None = None
...
class Q5TurnDiagnostics:
    ...
    proposed_exploration_coverage: dict[str, Any] = field(default_factory=dict)
    proposed_agent_plan: dict[str, Any] | None = None
    validated_agent_plan: dict[str, Any] | None = None
    effective_question_plan: dict[str, Any] | None = None
    assessor_called: bool = False
    assessor_attempt_count: int = 0
    assessor_validation_failures: list[str] = field(default_factory=list)
    assessor_assessment: dict[str, Any] | None = None
...
_agent_models: dict[str, Any] = {}
_assessor_model: Any | None = None
...
def _get_agent_model(
    *_: Any,
    ...
    model = _agent_models.get("coverage")
    if model is None:
        tools = [
            ASK_QUESTION_TOOL,
            REQUEST_EXAMPLE_TOOL,
            READY_FOR_ASSESSMENT_TOOL,
            USER_SELECTED_DIRECTION_TOOL,
        ]
        ...
        _agent_models["coverage"] = model
    return model


def _get_assessor_model() -> Any:
    global _assessor_model
    if _assessor_model is None:
        _assessor_model = _get_llm().with_structured_output(
            CompletionAssessmentEnvelope,
            method="json_schema",
            strict=True,
        )
    return _assessor_model
...
def _requires_user_selected_direction(
    ...
    unclear_attempts = [
        ...
        if item.answer_disposition == AnswerDisposition.UNCLEAR
    ]
    if len(unclear_attempts) < 2:
        return False
    previous, current = unclear_attempts[-2:]
    previous_dimensions = BLOCKED_SIGNATURE_DIMENSIONS[
        previous.semantic_route_type
    ]
    current_dimensions = BLOCKED_SIGNATURE_DIMENSIONS[
...
def _build_agent_payload(
    ...
    coverage_review_required = bool(
        runtime.pending_assessment is None
    )
    return {
        "mode": mode,
        ...
        "completionAssessmentAllowed": False,
        ...
        "pendingAssessmentGap": (
            runtime.pending_assessment_gap.model_dump(by_alias=True, mode="json")
            if runtime.pending_assessment_gap is not None
            else None
        ),
...
def _parse_agent_action(message: Any) -> CoverageAgentAction:
...
_Q10_HARM_TO_OTHERS = re.compile(
    r"(?:다치게|때리|해치|죽이|보복하).{0,18}(?:고\s*싶|할\s*생각|할\s*계획)"
    r"|(?:찾아가(?:서|\s)).{0,20}(?:다치게|때리|해치|죽이|보복)"
)
_Q10_SELF_HARM = re.compile(
    r"(?:나를|내\s*(?:몸|목숨)|스스로).{0,22}(?:다치게|해치|죽이|칼로|자해)"
    r"|(?:지금.{0,15})?(?:자해|죽고\s*싶|목숨을\s*끊).{0,20}(?:지금|생각|계획|하려|어|다)"
)


def _q10_latest_safety_candidate(request: CbtRequest) -> dict[str, Any] | None:
    """Latest current-user clause만 보는 bounded Q10 safety detector입니다."""

    history = _history_for(request)
    sources = [history[-1].answer] if history else [
        request.record.automatic_thought,
        request.record.situation or "",
    ]
    for raw in sources:
        ...
        if not text:
            continue
        lowered = text.lower()
        # scope/인용·가정·창작을 current-user intent보다 먼저 제외합니다.
        if any(marker in lowered for marker in (
            "만약", "말하면", "친구가", "동료가", "그 사람이", "누군가가",
            "소설", "영화", "대사", "인물이", "교육", "예시", "인용",
        )):
            continue
        # 명시적 부정과 종료된 과거를 current intent보다 먼저 제외합니다.
        if any(marker in lowered for marker in (
            "생각은 없다", "생각이 없다", "싶지 않", "지금은 아니다",
            "지금은 그런 생각이 없", "예전에는", "과거에는", "지난해",
        )):
            continue
        if _Q10_HARM_TO_OTHERS.search(lowered):
            return {
                "reason": RiskReasonCode.HARM_TO_OTHERS.value,
                "evidence": text[:300],
                "sourceType": "LATEST_USER_ANSWER" if history else "RECORD",
                "subject": "CURRENT_USER",
                "currentness": "CURRENT",
            }
        if _Q10_SELF_HARM.search(lowered):
            reason = (
                RiskReasonCode.SUICIDE
                if any(marker in lowered for marker in ("죽고 싶", "목숨을 끊", "죽일 생각"))
                else RiskReasonCode.SELF_HARM
            )
            return {
                "reason": reason.value,
                "evidence": text[:300],
    ...
    if history:
        return None
    # START record에서는 기존 bounded detector가 잡는 명시적 current risk도 보존합니다.
    candidates = _safety_candidates(request)
    return candidates[0] if candidates else None


def _safety_stop_draft(
    request: CbtRequest,
    state: CbtSessionState,
    candidate: dict[str, Any],
) -> CbtAnalysisDraft:
    def representative(items: list[SavedAnswerEvidence]) -> SavedAnswerEvidence | None:
        return items[-1] if items else None

    reason = RiskReasonCode(candidate["reason"])
    draft = CbtAnalysisDraft(
        ...
        confirmation=None,
        risk=RiskAssessment(
            level=(RiskLevel.CRISIS if reason == RiskReasonCode.IMMEDIATE_DANGER else RiskLevel.REVIEW),
            reason_code=reason,
        ),
        safety_evidence=candidate["evidence"],
    )
    sources = [item.answer or "" for item in _history_for(request)] + [
        request.record.automatic_thought,
        request.record.situation or "",
    ]
    if not any(candidate["evidence"] in source for source in sources):
        raise CbtDraftValidationError("Server safety evidence must be exact input text")
    return draft
...
def _build_assessor_payload(
    ...
    valid_coverage = coverage.model_dump(by_alias=True, mode="json")
    rejected = {
        item.code
        for item in request.before_distortions
        if item.review_status.value == "REJECTED"
    ...
    return {
        ...
        "normalizedCoverage": valid_coverage,
        "savedAnswers": [
            _question_dump(item)
            for item in request.question_answers
            if _session_answer_disposition(item)
            not in {
                AnswerDisposition.DIALOGUE_CONTROL,
                AnswerDisposition.UNCLEAR,
                AnswerDisposition.SKIPPED,
        ...
        "rejectedDistortionCodes": sorted(code.value for code in rejected),
        "pendingAssessmentGap": (
            runtime.pending_assessment_gap.model_dump(by_alias=True, mode="json")
            if runtime.pending_assessment_gap is not None
            else None
...
def _validate_assessment_grounding(
    ...
    history = {item.question_code: item for item in request.question_answers}
    if isinstance(assessment, NeedsMoreExplorationAssessment):
        grounding_codes = assessment.assessment_gap.grounding_question_codes
    else:
        grounding_codes = assessment.grounding_question_codes
        if assessment.automatic_thought_excerpt not in request.record.automatic_thought:
            raise CbtDraftValidationError(
                "automaticThoughtExcerpt must be an exact non-empty substring"
            )
        grounded_answers = [
            item.answer or ""
            for item in request.question_answers
        ...
        if not any(assessment.observed_fact in answer for answer in grounded_answers):
            raise CbtDraftValidationError(
                "observedFact must be an exact saved USER answer excerpt"
            )
    if not grounding_codes:
        raise CbtDraftValidationError("Assessment groundingQuestionCodes are required")
    for code in grounding_codes:
        source = history.get(code)
        if source is None or source.answer is None:
            raise CbtDraftValidationError(
                "Assessment groundingQuestionCodes must exist in saved history"
            )
        if _session_answer_disposition(source) in {
            AnswerDisposition.DIALOGUE_CONTROL,
            AnswerDisposition.UNCLEAR,
            ...
            raise CbtDraftValidationError(
                "Assessment cannot use dialogue-control, unclear, or skipped answers"
            )
    ...
    if isinstance(assessment, DistortionPresentAssessment):
        if assessment.matched_distortion_code in rejected:
            raise CbtDraftValidationError("A rejected distortion cannot be re-proposed")
    elif isinstance(assessment, NoClearDistortionAssessment):
        for candidate in assessment.considered_candidates:
            if candidate.code in rejected:
                raise CbtDraftValidationError("A rejected distortion cannot be reconsidered")
            exclusion = (
                candidate.missing_definition_element
                + " "
                + candidate.why_automatic_thought_does_not_match
            ).lower()
            if any(marker in exclusion for marker in (
                "외부 증거가 없", "뒷받침하는 증거가 없", "증거가 부족", "근거가 부족",
            )):
                raise CbtDraftValidationError(
                    "Lack of external evidence cannot exclude a distortion definition"
                )
    else:
        gap_text = (
            assessment.assessment_gap.why_existing_answers_are_insufficient
            + " "
            + assessment.assessment_gap.distinction_needed
        ).lower()
        if any(marker in gap_text for marker in (
            "확신이 낮", "모든 왜곡", "부재를 증명", "같은 질문",
        )):
            raise CbtDraftValidationError("Assessment gap is not a specific close boundary")
        if assessment.answer_source not in ANSWER_SOURCES_BY_ROUTE[assessment.recommended_route]:
            raise CbtDraftValidationError("Assessment gap answerSource is incompatible with route")
...
async def _assess_completion(
    ...
    *,
    ...
    payload = _build_assessor_payload(request, coverage, runtime)
    model = assessor_model or _get_assessor_model()
    feedbacks: list[str] = []
    diagnostics.assessor_called = True
    for attempt in range(CBT_ASSESSOR_MODEL_OUTPUT_ATTEMPTS):
        diagnostics.assessor_attempt_count = attempt + 1
        messages = [SystemMessage(content=COMPLETION_ASSESSOR_SYSTEM_PROMPT)]
        if feedbacks:
            messages.append(SystemMessage(content=(
                "Previous assessment failed these structural validations:\n- "
                + "\n- ".join(feedbacks)
                + "\nReturn one corrected assessment from the same saved evidence."
            )))
        messages.append(HumanMessage(content=json.dumps(payload, ensure_ascii=False)))
        try:
            raw = await model.ainvoke(messages)
            envelope = CompletionAssessmentEnvelope.model_validate(raw)
            assessment = _COMPLETION_ASSESSMENT_ADAPTER.validate_python(
                envelope.assessment.model_dump(by_alias=True, mode="json")
            )
            _validate_assessment_grounding(assessment, request)
        except ValidationError as exc:
            feedback = ", ".join(
                f"{'.'.join(str(part) for part in error['loc'])}: {error['msg']}"
                for error in exc.errors(include_input=False)
            )[:1_000]
        except (OutputParserException, CbtDraftValidationError) as exc:
            feedback = str(exc)[:1_000]
        else:
            diagnostics.assessor_assessment = assessment.model_dump(by_alias=True, mode="json")
            return assessment
        feedbacks.append(feedback)
        diagnostics.assessor_validation_failures.append(feedback)
    raise CbtModelOutputExhaustedError("completion assessment", feedbacks) from None


def _render_distortion_assessment(
    assessment: DistortionPresentAssessment,
    request: CbtTurnRequest,
    coverage: ExplorationCoverage,
    diagnostics: Q5TurnDiagnostics,
) -> CbtTurnResponse:
    acknowledgement = coverage.acknowledgement.grounding
    assert acknowledgement is not None
    proposal = DistortionProposal(code=assessment.matched_distortion_code, classifier_confidence=1.0)
    definition = DISTORTION_DEFINITIONS[assessment.matched_distortion_code]
    message = "\n".join((
        _coverage_line("처음 생각을 지지하는 방향", coverage.evidence_for),
        _coverage_line("확신을 낮추는 방향", coverage.evidence_against),
        _coverage_line("가능한 다른 관점", coverage.alternative_views),
        _coverage_line("사실과 추론·확신을 나눈 내용", coverage.acknowledgement),
        f"잠정적으로 살펴볼 인지 왜곡: {definition['nameKo']} ({assessment.matched_distortion_code.value})",
        f"균형 잡힌 생각: {assessment.calibrated_thought}",
    ))[:4_000]
    response = CbtTurnResponse(
        request_id=request.request_id,
        status=CbtApiStatus.CONFIRM_REQUIRED,
        assessment_type=CbtAssessmentType.DISTORTION_PRESENT,
        next_question=None,
        before_distortions=[proposal],
        outcome_draft=ReflectionOutcomeDraft(
            evidence_for_text=(coverage.evidence_for.grounding.excerpt if coverage.evidence_for.status == ExplorationStatus.EVIDENCE_FOUND else None),
            evidence_against_text=(coverage.evidence_against.grounding.excerpt if coverage.evidence_against.status == ExplorationStatus.EVIDENCE_FOUND else None),
            alternative_thought_text=assessment.calibrated_thought,
            after_distortions=[proposal],
        ),
        confirmation_required_fields=list(CONFIRMATION_REQUIRED_FIELDS),
        acknowledgement_evidence=acknowledgement.excerpt,
        acknowledgement_source_question_code=acknowledgement.source_question_code,
        proposal_message=message,
        risk=RiskAssessment(level=RiskLevel.NONE, reason_code=None),
        meta=AnalysisMeta(model=CBT_MODEL, prompt_version=CBT_AGENT_PROMPT_VERSION),
    )
    diagnostics.render_source = "DETERMINISTIC_DISTORTION_ASSESSMENT"
    diagnostics.deterministic_renderer_output = response.model_dump(by_alias=True, mode="json")
    return response


def _render_no_clear_assessment(
    assessment: NoClearDistortionAssessment,
    request: CbtTurnRequest,
    coverage: ExplorationCoverage,
    diagnostics: Q5TurnDiagnostics,
) -> CbtTurnResponse:
    ...
    message = "\n".join((
        ...
        _coverage_line("다른 관점", coverage.alternative_views),
        _coverage_line("사실과 생각을 나눈 내용", coverage.acknowledgement),
        f"기록된 사실: {assessment.observed_fact}",
        "현재 저장된 답변과 제공된 정의만으로는 특정 인지 왜곡이 뚜렷하게 근거화되지는 않았습니다. 이 평가는 현실의 진위를 확정하는 판단이 아닙니다.",
        f"조정해 본 생각: {assessment.calibrated_thought}",
        "지금까지 확인한 내용을 이대로 정리해도 괜찮을까요?",
    ))[:4_000]
    response = CbtTurnResponse(
        request_id=request.request_id,
    ...
    diagnostics.render_source = "DETERMINISTIC_NO_CLEAR_ASSESSMENT"
    diagnostics.deterministic_renderer_output = response.model_dump(by_alias=True, mode="json")
    return response


_Q10_PURPOSE_BY_ROUTE: dict[SemanticRouteType, QuestionPurpose] = {
    SemanticRouteType.OBSERVABLE_EVENT_DETAIL: QuestionPurpose.SITUATION_REFLECTION,
    SemanticRouteType.DIRECT_WORD_OR_ACTION: QuestionPurpose.EVIDENCE_FOR,
    SemanticRouteType.EXPECTED_SIGNAL_ABSENCE: QuestionPurpose.EVIDENCE_AGAINST,
    SemanticRouteType.CONTRADICTORY_FACT: QuestionPurpose.EVIDENCE_AGAINST,
    SemanticRouteType.OTHER_PEOPLE_COMPARISON: QuestionPurpose.EVIDENCE_AGAINST,
    SemanticRouteType.CERTAINTY_REASSESSMENT: QuestionPurpose.BALANCED_THOUGHT,
    SemanticRouteType.ALTERNATIVE_EXPLANATION: QuestionPurpose.ALTERNATIVE_VIEW,
    SemanticRouteType.BALANCED_CONCLUSION: QuestionPurpose.BALANCED_THOUGHT,
    SemanticRouteType.EMOTION_OR_TRIGGER: QuestionPurpose.EMOTION_REFLECTION,
    SemanticRouteType.USER_SELECTED_DIRECTION: QuestionPurpose.FREE_REFLECTION,
}


def _render_assessment_gap_question(
    assessment: NeedsMoreExplorationAssessment,
    diagnostics: Q5TurnDiagnostics,
) -> tuple[AgentQuestionPlan, QuestionWordingDraft]:
    target = _clean_deterministic_text(assessment.answer_target)
    question = f"판단 경계를 구분하기 위해 {target}에 대해 확인할 수 있는 내용을 말씀해 주실 수 있나요?"
    plan = AgentQuestionPlan(
        question_purpose=_Q10_PURPOSE_BY_ROUTE[assessment.recommended_route],
        semantic_route_type=assessment.recommended_route,
        latest_user_intent=LatestUserIntent.CBT_ANSWER,
        question_goal=assessment.question_goal,
        answer_target=assessment.answer_target,
        answer_source=assessment.answer_source,
        preface_goal=None,
        grounding_question_codes=assessment.assessment_gap.grounding_question_codes[:5],
        avoid_topics=[],
        example_options=[],
    )
    diagnostics.render_source = "DETERMINISTIC_ASSESSMENT_GAP"
    diagnostics.deterministic_question = question
    diagnostics.effective_question_plan = plan.model_dump(by_alias=True, mode="json")
    return plan, QuestionWordingDraft(preface=None, question=question)


def _validate_agent_action(
    action: CoverageAgentAction,
    request: CbtRequest,
    ...
    *,
    ...
    if isinstance(action, ReadyForAssessmentAction):
        if not normalized_state.exploration_coverage.is_complete():
            raise CbtDraftValidationError(
                "READY_FOR_ASSESSMENT requires complete normalized coverage"
            )
        return None

    if not isinstance(
        action,
        (AskQuestionAction, RequestExampleAction, UserSelectedDirectionAction),
    ):
        raise CbtDraftValidationError("Unsupported Coverage Agent action")

    if isinstance(action, RequestExampleAction):
        if action.question_plan.latest_user_intent != LatestUserIntent.REQUEST_EXAMPLE:
            raise CbtDraftValidationError("REQUEST_EXAMPLE action requires REQUEST_EXAMPLE intent")
    if isinstance(action, UserSelectedDirectionAction):
        if action.question_plan.semantic_route_type != SemanticRouteType.USER_SELECTED_DIRECTION:
            raise CbtDraftValidationError("USER_SELECTED_DIRECTION action requires its matching route")
    ...
    if not incomplete_domains:
        # 같은 Agent 호출의 valid coverageReview가 complete를 만들면 raw ask plan은
        # 실행하지 않고 Assessor로 전환합니다.
        return None
    ...
    if route_type in normalized_state.blocked_routes:
        raise CbtDraftValidationError(
            "Agent questionPlan.semanticRouteType reuses a blocked state route"
        )
    if normalized_state.direct_support_closed and route_type in {
        SemanticRouteType.DIRECT_WORD_OR_ACTION,
        SemanticRouteType.EXPECTED_SIGNAL_ABSENCE,
    }:
        raise CbtDraftValidationError(
    ...
    if action.question_plan.answer_source not in ANSWER_SOURCES_BY_ROUTE[route_type]:
        raise CbtDraftValidationError(
            "answerSource is incompatible with semanticRouteType"
        )
    target_domain = _question_plan_exploration_domain(action.question_plan)
    if (
        route_type != SemanticRouteType.USER_SELECTED_DIRECTION
        and target_domain not in incomplete_domains
    ):
        raise CbtDraftValidationError(
            "Coverage question must target an actual NOT_EXPLORED domain"
        )
    draft = CbtAnalysisDraft(
        result_type=CbtResultType.QUESTION,
...
async def _select_agent_action(
    ...
    *,
    ...
    diagnostics: Q5TurnDiagnostics,
) -> tuple[CoverageAgentAction, CbtAnalysisDraft | None, CbtSessionState]:
    payload = _build_agent_payload(request, runtime, mode)
    available_routes = {
        SemanticRouteType(item["semanticRouteType"])
        for item in payload["semanticRouteDefinitions"]
    }
    diagnostics.available_tools = list(ACTION_SCHEMAS)
    ...
    for attempt in range(CBT_AGENT_MODEL_OUTPUT_ATTEMPTS):
        ...
        diagnostics.completion_assessment_allowed = floor_complete
        ...
        try:
            ...
            diagnostics.proposed_exploration_coverage = action.coverage_review.model_dump(by_alias=True, mode="json")
            explicit_intent = _explicit_feedback_intent(request)
            if isinstance(action, (AskQuestionAction, RequestExampleAction, UserSelectedDirectionAction)):
                diagnostics.proposed_agent_plan = action.question_plan.model_dump(by_alias=True, mode="json")
            if isinstance(action, (AskQuestionAction, RequestExampleAction, UserSelectedDirectionAction)) and explicit_intent is not None:
                action.question_plan = action.question_plan.model_copy(
                    update={"latest_user_intent": explicit_intent}
                )
            history = _history_for(request)
            if (
                isinstance(action, (AskQuestionAction, RequestExampleAction, UserSelectedDirectionAction))
                and history
            ...
            floor_complete = verified_state_floor.exploration_coverage.is_complete()
            # complete review + raw ask는 retry하지 않고 Assessor로 전환합니다.
            if floor_complete and not isinstance(action, ReadyForAssessmentAction):
                action = ReadyForAssessmentAction(coverage_review=action.coverage_review)
            ...
            diagnostics.selected_tool = (
                ...
                else REQUEST_EXAMPLE_TOOL.name
                if isinstance(action, RequestExampleAction)
                else READY_FOR_ASSESSMENT_TOOL.name
                if isinstance(action, ReadyForAssessmentAction)
                else USER_SELECTED_DIRECTION_TOOL.name
            )
            if isinstance(action, (AskQuestionAction, RequestExampleAction, UserSelectedDirectionAction)):
                diagnostics.validated_agent_plan = action.question_plan.model_dump(by_alias=True, mode="json")
            if CBT_DEBUG_LOG_ANALYSIS:
                logger.info(
...
async def _run_q10_turn(
    ...
    *,
    ...
    """Q10 고정 순서: safety → coverage → Writer 또는 Assessor."""

    _refresh_unanswered_question_attempts(request, runtime, mode)
    diagnostics = Q5TurnDiagnostics(mode=mode)
    runtime.last_diagnostics = diagnostics
    state_for_runtime = _normalize_agent_state(runtime.state, request, runtime)

    safety_candidate = _q10_latest_safety_candidate(request)
    agent_plan: AgentQuestionPlan | None = None
    assessment_gap: AssessmentGap | None = None
    if safety_candidate is not None:
        diagnostics.selected_tool = "SERVER_SAFETY_STOP"
        diagnostics.render_source = "DETERMINISTIC_SERVER_SAFETY"
        diagnostics.deterministic_renderer_input = dict(safety_candidate)
        draft = _safety_stop_draft(request, state_for_runtime, safety_candidate)
        response = _with_agent_meta(_to_response(draft, request, None))
        diagnostics.deterministic_renderer_output = response.model_dump(by_alias=True, mode="json")
    else:
        try:
            action, draft, state_for_runtime = await _select_agent_action(
            ...
            state_for_runtime = getattr(exc, "verified_state_floor", state_for_runtime)
            progress = CbtSemanticProgress(
                evidence_for=(state_for_runtime.evidence_for[-1] if state_for_runtime.evidence_for else None),
                evidence_against=(state_for_runtime.evidence_against[-1] if state_for_runtime.evidence_against else None),
                alternative_view=(state_for_runtime.alternative_views[-1] if state_for_runtime.alternative_views else None),
                acknowledgement=state_for_runtime.acknowledgement,
            )
            draft, wording = _build_deterministic_fallback(request, semantic_progress=progress)
            draft, wording = _constrain_agent_fallback(request, runtime, draft, wording)
            diagnostics.fallback_used = True
            diagnostics.fallback_reason = f"{exc.stage}: {'; '.join(exc.feedbacks)}"
            diagnostics.render_source = "DETERMINISTIC_COVERAGE_FALLBACK"
            response = _with_agent_meta(_to_response(draft, request, wording))
        else:
            coverage = state_for_runtime.exploration_coverage
            diagnostics.exploration_coverage = coverage.model_dump(by_alias=True, mode="json")
            if coverage.is_complete():
                if not isinstance(request, CbtTurnRequest):
                    raise CbtDraftValidationError("Complete coverage assessment requires TURN")
                assessment = await _assess_completion(
                    request,
                    coverage,
                    runtime,
                    diagnostics,
                ...
                if isinstance(assessment, DistortionPresentAssessment):
                    diagnostics.assessment_type = "DISTORTION_PRESENT"
                    response = _render_distortion_assessment(assessment, request, coverage, diagnostics)
                elif isinstance(assessment, NoClearDistortionAssessment):
                    diagnostics.assessment_type = "NO_CLEAR_DISTORTION"
                    response = _render_no_clear_assessment(assessment, request, coverage, diagnostics)
                else:
                    diagnostics.assessment_type = "NEEDS_MORE_EXPLORATION"
                    assessment_gap = assessment.assessment_gap
                    agent_plan, wording = _render_assessment_gap_question(assessment, diagnostics)
                    draft = CbtAnalysisDraft(
                        result_type=CbtResultType.QUESTION,
                        semantic_progress=CbtSemanticProgress(
                            evidence_for=_positive_coverage_grounding(coverage.evidence_for),
                            evidence_against=_positive_coverage_grounding(coverage.evidence_against),
                            alternative_view=_positive_coverage_grounding(coverage.alternative_views),
                            acknowledgement=_positive_coverage_grounding(coverage.acknowledgement),
                        ),
                        question_plan=_to_shared_question_plan(agent_plan),
                        confirmation=None,
                        risk=RiskAssessment(level=RiskLevel.NONE, reason_code=None),
                        safety_evidence=None,
                    )
                ...
                if not isinstance(action, (AskQuestionAction, RequestExampleAction, UserSelectedDirectionAction)):
                    raise CbtDraftValidationError("Incomplete coverage requires a question action")
                assert draft is not None
                agent_plan = action.question_plan
                diagnostics.answer_target = agent_plan.answer_target
                diagnostics.answer_source = agent_plan.answer_source.value
                diagnostics.evidence_polarity = _evidence_polarity(agent_plan).value
                shared_plan = _apply_feedback_constraints(request, draft.question_plan)
                if shared_plan is not draft.question_plan:
                    draft = draft.model_copy(update={"question_plan": shared_plan})
                if isinstance(action, RequestExampleAction) or agent_plan.latest_user_intent == LatestUserIntent.REQUEST_EXAMPLE:
                    wording = _render_example_question(agent_plan, diagnostics)
                elif (
                    state_for_runtime.direct_support_closed
                    and shared_plan.question_purpose == QuestionPurpose.EVIDENCE_AGAINST
                    and shared_plan.semantic_route_type == SemanticRouteType.CONTRADICTORY_FACT
                ):
                    wording = _render_evidence_against_after_no_direct(diagnostics)
                else:
                    try:
                        wording = await _write_q5_question(
                            request,
                        ...
                        wording = _render_answer_target_fallback(agent_plan, diagnostics)
                        diagnostics.fallback_used = True
                        diagnostics.fallback_reason = f"{exc.stage}: {'; '.join(exc.feedbacks)}"
                diagnostics.effective_question_plan = {
                    **agent_plan.model_dump(by_alias=True, mode="json"),
                    "questionPurpose": shared_plan.question_purpose.value,
                    "semanticRouteType": shared_plan.semantic_route_type.value,
                }
                response = _with_agent_meta(_to_response(draft, request, wording))
    ...
    if response.status == CbtApiStatus.CONTINUE:
        ...
        runtime.pending_plan = agent_plan
        runtime.pending_assessment = None
        runtime.pending_assessment_gap = assessment_gap
    else:
        runtime.state = state_for_runtime
        runtime.history = list(_history_for(request))
        runtime.pending_question = None
        runtime.pending_plan = None
        runtime.pending_assessment_gap = None
        if response.status == CbtApiStatus.CONFIRM_REQUIRED:
            assert response.assessment_type is not None
            assert response.outcome_draft is not None
            assert response.proposal_message is not None
