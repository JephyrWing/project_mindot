...
def source_files():
    ...
    files += [OUT/'openapi.json',OUT/'offline-receipt.json',OUT/'review.md']
    files += list((OUT/'package/docs/cbt-q11-evaluation').glob('*'))
...
def lock():
    ...
    files=[dict(path=str(p.relative_to(AI)),bytes=p.stat().st_size,sha256=sha(p.read_bytes())) for p in source_files()]
    value=dict(revision='simple-dialogue-1',createdAt=datetime.now(timezone.utc).isoformat(),files=files,
        ...
        knownCeilings=schedule['ceilings'],formalSeed=20260908,modelSeed='NOT_APPLIED_BOTH_VERSIONS',
...
async def canary():
    ...
    try:
        for case in plan['cases']:
            ...
            if req is None:
                row=dict(caseId=case['id'],status='BLOCKED_BY_PARENT',request=None,response=None,error=None)
            else:
                ...
                row=dict(caseId=case['id'],status='TECHNICAL_FAIL' if technical else 'REQUIRES_FUNCTION_REVIEW',request=req,response=response,
                    error=error,technicalFailures=technical,elapsedSeconds=time.perf_counter()-started,acceptedState=state,
