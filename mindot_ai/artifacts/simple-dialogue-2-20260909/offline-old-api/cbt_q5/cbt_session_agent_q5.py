...
CBT_AGENT_PROMPT_VERSION = "cbt-session-agent-quality-q5"
...
Update compact state and choose safety, confirmation, or one question plan.
The Writer handles wording except server-rendered cases. Do not diagnose,
invent facts, or prove automaticThought false.
</role>
...
eligibility. safetyCandidates, confirmationAllowed, directSupportClosed,
blockedRoutes, blockedSemanticDimensions, and exclusions are authoritative.
Select only a supplied route and never create OTHER_SPECIFIC.
</input>
...
NO_DIRECT_EVIDENCE is never evidenceFor. It sets directSupportClosed, so do not
seek direct support through another person, time, wording, or route.

Relevance or repetition feedback is not evidence. Preserve all exclusions and
do not reuse a blocked target, comparison, cause, evidence source, assumed
causal link, or information request through another route. Example,
explanation, and difficulty requests are dialogue control.
</state>
...
questionGoal must express one meaning. questionGoal is a Korean plan, not final
wording. answerTarget names exactly one piece of information the USER can
...
blocked material. Call one available tool and never return COMPLETE.
</plan>
""".strip()
...
async def _select_agent_action(
    ...
    *,
    ...
    if confirmation_allowed:
        diagnostics.available_tools.append(REQUEST_CONFIRMATION_TOOL.name)
