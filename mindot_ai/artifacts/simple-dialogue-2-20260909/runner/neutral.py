...
def nullable(node):
    ...
    return node
SCHEMA['properties']['input']['properties'].update(allowedDecisions=nullable(old.array(old.TEXT)),
    classificationApplicability=old.TEXT,assessmentPolicy=old.TEXT)
SCHEMA['properties']['input']['required']+=['allowedDecisions','classificationApplicability','assessmentPolicy']
...
def export(implementation,anonymous,case_id,turn_index,request,response,state,context=None,observations=None):
    ...
    for key in ('allowedDecisions','classificationApplicability','assessmentPolicy'):
        payload['input'][key]=deepcopy(context.get(key))
    old.validate(payload,SCHEMA)
    # Added text is original USER content, not metadata to redact.
    old.NATURAL_LANGUAGE_KEYS=old.NATURAL_LANGUAGE_KEYS|{'text','note','assessmentPolicy'}
