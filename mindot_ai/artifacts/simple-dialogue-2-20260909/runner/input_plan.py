...
FORMAT_VERSION = "formal-input-plan-2"
KNOWN_FILES = {
...
REVISION_MANIFEST=json.loads((Path(__file__).resolve().parents[1]/'package/docs/cbt-q11-evaluation/known-revision-manifest.json').read_text(encoding='utf-8'))
KNOWN_FILES={Path(f['path']).name:(f['recordCount'],f['bytes'],f['sha256']) for f in REVISION_MANIFEST['files']}
...
LONG_KEYS = {"sessionCaseId", "versions", "flow", "recordSourceCaseId", "freshSourceCaseId", "maxTurns",
             ...
             "evaluationContext"}
SINGLE_KEYS |= {'allowedDecisions','classificationApplicability','assessmentPolicy'}
...
def grading_context(case, common_definitions):
    ...
    for key in ('allowedDecisions','classificationApplicability','assessmentPolicy'):
...
def load_known(directory):
    ...
    multiple=[c['original']['caseId'] for c in result['cases'] if c['original'].get('classificationApplicability')=='MULTIPLE_VALID_ACTIONS']
    _require(set(multiple)==set(REVISION_MANIFEST['multiActionCaseIds']),'multi_action_ids_mismatch')
    for case in result['cases']:
        r=case['original']
        if r.get('classificationApplicability')=='MULTIPLE_VALID_ACTIONS':
            _require(r['expectedDecision'] is None and r['expectedAssessment'] is None and isinstance(r['allowedDecisions'],list),'multi_action_nullable_contract')
    result['knownRevision']=REVISION_MANIFEST['revision']
    result['classificationSidecar']={
        'multipleValidActions':sorted(multiple),
        'fixedSingleExpectedAction':sorted(c['original']['caseId'] for c in result['cases'] if c['kind']=='single' and c['gradingContext']['expectedDecision'] is not None),
        'goldSafetyCases':sorted(c['original']['caseId'] for c in result['cases'] if c['kind']=='single' and str(c['gradingContext']['expectedDecision']).startswith('SAFETY')),
        'goldByCase':{c['caseKey']:{k:deepcopy(c['gradingContext'].get(k)) for k in ('expectedDecision','expectedAssessment','allowedDecisions','classificationApplicability')} for c in result['cases']},
        'policy':'Predetermined gold groups only; no removal based on actual STOP/safety/null/errors. GPT applies its locked diagnostic formula; all cases remain in collection.'}
    return result
